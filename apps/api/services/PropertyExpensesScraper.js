...
class PropertyExpensesScraper {
                ...
                if (keyElement.length && valueElement.length) {
                    ...
                    switch (key) {
                        ...
                        case 'municipal rates and taxes':
                            ...
                            logger.info(`Split municipal_rates_taxes: rates=${expenses.municipal_rates}, taxes=${expenses.municipal_taxes}`);
                            break;
                    }
                }
            });

            // Estimate insurance based on property characteristics if not found
            if (expenses.insurance_estimate === 0) {
                expenses.insurance_estimate = this.estimateInsurance($);
            }

            // Estimate maintenance reserve if not found (typically 1% of property value annually, so ~0.08% monthly)
            if (expenses.maintenance_reserve === 0) {
                expenses.maintenance_reserve = this.estimateMaintenanceReserve($);
            }
    ...
    /**
     * Estimate insurance based on property type and value
     * @param {CheerioAPI} $ - The cheerio instance
     * @returns {number} Estimated monthly insurance
     */
    estimateInsurance($) {
        try {
            // Look for property price to estimate insurance
            const priceElement = $('.p24_price');
            if (priceElement.length > 0) {
                const priceText = priceElement.text();
                const price = this.extractCurrency(priceText);
                
                if (price > 0) {
                    // Rough estimate: 0.3-0.5% of property value annually
                    // For monthly: (price * 0.004) / 12
                    return Math.round((price * 0.004) / 12);
                }
            }

            // Default fallback estimate
            return 500; // R500 per month default estimate
        } catch (error) {
            logger.error(`Error estimating insurance: ${error.message}`);
            return 500;
        }
    }
     ...
     * @param {CheerioAPI} $ - The cheerio instance
     * @returns {number} Estimated monthly maintenance reserve
     */
    estimateMaintenanceReserve($) {
        try {
            // Look for property price to estimate maintenance
            const priceElement = $('.p24_price');
            if (priceElement.length > 0) {
                const priceText = priceElement.text();
                const price = this.extractCurrency(priceText);
                ...
                if (price > 0) {
                    // Rough estimate: 1% of property value annually
                    // For monthly: (price * 0.01) / 12
                    return Math.round((price * 0.01) / 12);
                }
            }

            // Default fallback estimate
            return 800; // R800 per month default estimate
        } catch (error) {
            logger.error(`Error estimating maintenance: ${error.message}`);
            return 800;
        }
    }
