...
export default function RentalPropertiesPage() {
  ...
  const handleFilterChange = (newFilters: Partial<IRentalPropertyFilters>) => {
    setFilters(prev => ({
      ...prev,
      ...newFilters,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="md:flex md:items-center md:justify-between">
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-1 text-sm text-secondary-500">
            {data?.total ? `${data.total.toLocaleString()} rental properties available` : 'Browse available rental properties'}
          </p>
        </div>
      </div>

      {/* Filters */}
      <RentalPropertyFilters filters={filters} onFilterChange={handleFilterChange} />
      ...
      {/* Properties Grid */}
      {!isLoading && data?.rental_properties && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {data.rental_properties.map((property) => (
